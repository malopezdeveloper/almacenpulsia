...
stop_and_remove_units(){
  local svc unit
  for svc in "$CADDY_SERVICE" "$APP_SERVICE" "pulsia-inventario-storage-admin" "pulsia-inventario-continuous-backup"; do
...
remove_pulsia_files(){
  ...
  if command -v ufw >/dev/null 2>&1; then
    mapfile -t nums < <(ufw status numbered 2>/dev/null | sed -n 's/^\[[[:space:]]*\([0-9][0-9]*\)\].*PULSIA Inventario \(HTTP\|HTTPS\).*/\1/p' | sort -rn)
    local n
