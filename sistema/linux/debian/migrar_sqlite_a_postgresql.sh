...
APP_USER="$(systemctl show "$SERVICE" -p User --value 2>/dev/null || true)"; [[ -n "$APP_USER" && "$APP_USER" != root ]] || APP_USER="$(stat -c '%U' "$APP_ROOT")"; APP_GROUP="$(id -gn "$APP_USER")"
mkdir -p "$BACKUP"; chmod 0700 "$BACKUP"; cp -a "$SQLITE" "$BACKUP/inventario.sqlite3"; cp -a "$ENV" "$BACKUP/.env"; chown "$APP_USER:$APP_GROUP" "$BACKUP"; chmod 0700 "$BACKUP"
# Django se ejecuta como APP_USER: debe poder leer .env durante dump/migrate/check.
chown "$APP_USER:$APP_GROUP" "$ENV"; chmod 0600 "$ENV"
...
set -e
if [[ $M1 -ne 0 || $M2 -ne 0 || $M3 -ne 0 ]]; then cp -a "$BACKUP/.env" "$ENV"; chown "$APP_USER:$APP_GROUP" "$ENV"; chmod 0600 "$ENV"; systemctl restart "$SERVICE" 2>/dev/null || true; echo "Migración fallida; restaurada configuración SQLite. Backup: $BACKUP" >&2; exit 1; fi
# Verificación mínima antes de retirar SQLite.
sudo -u "$APP_USER" env DATABASE_URL="$DATABASE_URL" "$PY" manage.py shell -c 'from django.db import connection; c=connection.cursor(); c.execute("SELECT 1"); assert c.fetchone()[0] == 1'
mv "$SQLITE" "$BACKUP/inventario.sqlite3.original"; chown -R root:root "$BACKUP"; chmod 0700 "$BACKUP"; systemctl restart "$SERVICE"; systemctl is-active --quiet "$SERVICE"; echo "Migración PostgreSQL completada. Backup: $BACKUP"
