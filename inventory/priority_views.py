...
def priority_panel(request):
    if not _can_manage(request.user):
        return HttpResponseForbidden('Sólo Gestor y Administradores pueden gestionar prioridades.')
    if request.method == 'POST':
        order = get_object_or_404(CustomerOrder, pk=request.POST.get('order'), status='open')
        if order.name.casefold() == 'stock':
            messages.error(request, 'STOCK no puede establecerse como pedido prioritario.')
            return redirect('board_priorities')
        zone_id = (request.POST.get('zone') or '').strip()
        zone = get_object_or_404(ProductionZone, pk=zone_id, is_active=True) if zone_id else None
        priority, created = BoardPriority.objects.get_or_create(order=order, zone=zone, defaults={'created_by': request.user, 'active': True})
        if not created and not priority.active:
            priority.active = True; priority.created_by = request.user; priority.save(update_fields=['active', 'created_by'])
        ...
        messages.success(request, f'Prioridad activada: {order.name} · {zone.name if zone else "Todas las zonas"}.')
        return redirect('board_priorities')
...
def priority_disable(request, pk):
    if not _can_manage(request.user):
        return HttpResponseForbidden('Sólo Gestor y Administradores pueden gestionar prioridades.')
    priority = get_object_or_404(BoardPriority.objects.select_related('order', 'zone'), pk=pk)
    priority.active = False; priority.save(update_fields=['active'])
    ...
    messages.success(request, 'Prioridad retirada.')
    return redirect('board_priorities')
...
def move_unit_to_stock(request, unit_pk):
    if not _can_manage(request.user):
        return HttpResponseForbidden('Sólo Gestor y Administradores pueden sacar unidades de un pedido.')
    unit = get_object_or_404(OrderUnit.objects.select_related('order', 'physical_unit'), pk=unit_pk)
    stock = stock_order()
    if stock is None:
        messages.error(request, 'No existe el pedido permanente STOCK. Ejecuta las migraciones de la actualización.')
        return redirect('order_detail', pk=unit.order_id)
    if unit.order_id == stock.pk:
        messages.info(request, f'{unit.serial_number} ya pertenece a STOCK.')
    ...
    try:
        ...
        messages.error(request, f'{unit.serial_number} ya tiene un ciclo en STOCK y no se puede duplicar.')
        return redirect('order_detail', pk=source_id)
    messages.success(request, f'{unit.serial_number} retirada de {source_name} y enviada a STOCK.')
    return redirect('order_detail', pk=source_id)
