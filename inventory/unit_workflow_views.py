...
def start_unit_intervention(request):
 ...
 if not unit:
  ...
  if row:
   ...
   if not order_id:return render(request,'inventory/unit_intervention_confirm.html',{'serial_number':sn,'zone':zone,'source':'aiken','aiken':row,'orders':CustomerOrder.objects.filter(status='open').order_by('-id')})
   order=get_object_or_404(CustomerOrder,pk=order_id,status='open');physical,_=PhysicalUnit.objects.get_or_create(serial_number=sn,defaults={'brand':str(row.get('brand') or ''),'model':str(row.get('model') or ''),'processor':str(row.get('processor') or ''),'ram':str(row.get('ram') or ''),'disk':str(row.get('disk') or '')})
   unit,_=OrderUnit.objects.get_or_create(order=order,physical_unit=physical,defaults={'serial_number':sn,'aiken_lot':str(row.get('lot') or order.lot),'aiken_unit_id':str(row.get('id') or ''),'brand':str(row.get('brand') or physical.brand or order.brand),'model':str(row.get('model') or physical.model or order.model),'processor':str(row.get('processor') or physical.processor or order.processor),'ram':str(row.get('ram') or physical.ram or order.ram),'disk':str(row.get('disk') or physical.disk or order.disk)});source_name='aiken'
  else:
   ...
   order=get_object_or_404(CustomerOrder,pk=request.POST.get('order'),status='open');physical,_=PhysicalUnit.objects.get_or_create(serial_number=sn,defaults={'brand':request.POST.get('brand','').strip(),'model':request.POST.get('model','').strip(),'processor':request.POST.get('processor','').strip(),'ram':request.POST.get('ram','').strip(),'disk':request.POST.get('disk','').strip()})
   unit,_=OrderUnit.objects.get_or_create(order=order,physical_unit=physical,defaults={'serial_number':sn,'brand':request.POST.get('brand','').strip(),'model':request.POST.get('model','').strip(),'processor':request.POST.get('processor','').strip(),'ram':request.POST.get('ram','').strip(),'disk':request.POST.get('disk','').strip()});source_name='manual'
 intervention=UnitIntervention.objects.create(unit=unit,worker=request.user,zone=zone,source=source_name,source_snapshot=_snapshot(unit));messages.success(request,f'{unit.serial_number} abierta en {zone.name}. Pedido: {unit.order.name}.');return redirect('unit_workbench',intervention_pk=intervention.pk)
