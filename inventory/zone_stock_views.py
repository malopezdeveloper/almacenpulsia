...
def zone_stock(request):
    ...
    for loc in locations:
        ...
        by_zone.setdefault(loc.zone_id, []).append({
            'location': loc, 'unit': loc.unit, 'order': loc.unit.order,
            'working': bool(intervention and not intervention.finished_at),
            'recommended': destination, 'source_zone': intervention.zone if intervention else None,
        })
...
def finish_unit_intervention(request, intervention_pk):
    ...
    try:
        with transaction.atomic():
            intervention = (UnitIntervention.objects.select_for_update()
                            .select_related('unit', 'unit__physical_unit', 'zone')
                            .filter(pk=intervention_pk, worker=request.user).first())
            if intervention is None:
                return _finish_error(request, 'La intervención no existe o pertenece a otro técnico.', 404)
            if intervention.finished_at:
                return _finish_error(request, 'Esta unidad ya estaba finalizada. Recarga la pizarra.', 409)

            destination_id = (request.POST.get('destination_zone') or '').strip()
            destination = None
            if destination_id:
                if not destination_id.isdigit():
                    return _finish_error(request, 'El destino recibido no es válido.', 400)
                destination = ProductionZone.objects.filter(pk=int(destination_id), is_active=True).first()
                if destination is None:
                    return _finish_error(request, 'La zona de destino ya no existe o está desactivada.', 409)
            ...
            AuditLog.objects.create(
                ...
                object_type='OrderUnit', object_id=str(intervention.unit_id),
                details={
    ...
    success_text = (f'{intervention.unit.serial_number}: trabajo finalizado en {intervention.zone.name}; '
                    f'enviada al stock de {destination.name}.' if destination else
                    f'{intervention.unit.serial_number}: trabajo finalizado; permanece en el stock de {intervention.zone.name}.')
    if _ajax(request):
        return JsonResponse({
            'ok': True, 'serial_number': intervention.unit.serial_number,
            'intervention_id': intervention.pk,
            'destination_zone_id': destination.pk if destination else None,
    ...
    try:
        with transaction.atomic():
            intervention = (UnitIntervention.objects.select_for_update()
                            ...
                            .filter(pk=intervention_pk, worker=request.user).first())
            if intervention is None:
                return JsonResponse({'ok': False, 'error': 'La intervención no existe o pertenece a otro técnico.'}, status=404)
            if intervention.alerts.exists() or intervention.component_installations.exists() or intervention.repair_confirmations.exists():
                return JsonResponse({'ok': False, 'error': 'Esta fila ya tiene trazabilidad asociada y no puede borrarse.'}, status=409)
            details = {
                'serial_number': intervention.unit.serial_number,
                'intervention_id': intervention.pk, 'unit_id': intervention.unit_id,
                'order_id': intervention.unit.order_id, 'zone': intervention.zone.name,
                ...
                'duration_seconds': intervention.duration_seconds, 'source': intervention.source,
        ...
        return JsonResponse({'ok': False, 'error': f'Error al borrar: {exc.__class__.__name__}: {exc}'}, status=500)
