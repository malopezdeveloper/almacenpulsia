...
class PhysicalUnit(models.Model):
 serial_number=models.CharField(max_length=180,unique=True,db_index=True);brand=models.CharField(max_length=160,blank=True);model=models.CharField(max_length=180,blank=True);processor=models.CharField(max_length=180,blank=True);ram=models.CharField(max_length=80,blank=True);disk=models.CharField(max_length=80,blank=True);created_at=models.DateTimeField(auto_now_add=True)
 def __str__(self):return self.serial_number
class CustomerOrder(models.Model):
 STATUS=[('open','Abierto'),('closed','Cerrado')];VISUAL=[('green','Verde'),('brown','Marrón')]
 name=models.CharField(max_length=180,db_index=True);customer=models.ForeignKey(Customer,on_delete=models.PROTECT,related_name='orders');brand=models.CharField(max_length=160,blank=True);model=models.CharField(max_length=180,blank=True);lot=models.CharField(max_length=160,blank=True,db_index=True);processor=models.CharField(max_length=180,blank=True);ram=models.CharField(max_length=80,blank=True);disk=models.CharField(max_length=80,blank=True);status=models.CharField(max_length=12,choices=STATUS,default='open',db_index=True);visual_family=models.CharField(max_length=12,choices=VISUAL,default='green');closed_at=models.DateTimeField(null=True,blank=True);closed_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,null=True,blank=True,related_name='customer_orders_closed');created_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='customer_orders_created');created_at=models.DateTimeField(auto_now_add=True)
...
class OrderUnit(models.Model):
 ...
 class Meta:constraints=[models.UniqueConstraint(fields=['order','physical_unit'],name='one_unit_cycle_per_order')]
 def save(self,*a,**k):self.serial_number=self.physical_unit.serial_number;return super().save(*a,**k)
 def __str__(self):return self.serial_number
...
class ComponentReservation(models.Model):
 STATUS=[('active','Reservado'),('installed','Reservado e instalado'),('cancelled','Cancelada')];repair=models.ForeignKey(Repair,on_delete=models.PROTECT,related_name='component_reservations',null=True,blank=True);unit=models.ForeignKey(OrderUnit,on_delete=models.PROTECT,related_name='component_reservations',null=True,blank=True);component=models.ForeignKey(Component,on_delete=models.PROTECT,related_name='reservations');technician=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='component_reservations');installed_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='component_installations',null=True,blank=True);unit_serial_number=models.CharField(max_length=180,db_index=True);status=models.CharField(max_length=16,choices=STATUS,default='active',db_index=True);reserved_at=models.DateTimeField(auto_now_add=True);installed_at=models.DateTimeField(null=True,blank=True);resolved_at=models.DateTimeField(null=True,blank=True);observations=models.TextField(blank=True)
