...
class CustomerOrder(models.Model):
    name=models.CharField(max_length=180,db_index=True); customer=models.ForeignKey(Customer,on_delete=models.PROTECT,related_name='orders'); brand=models.CharField(max_length=160,blank=True); model=models.CharField(max_length=180,blank=True); lot=models.CharField(max_length=160,blank=True,db_index=True); processor=models.CharField(max_length=180,blank=True); ram=models.CharField(max_length=80,blank=True); disk=models.CharField(max_length=80,blank=True); created_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='customer_orders_created'); created_at=models.DateTimeField(auto_now_add=True)
    def __str__(self): return f'{self.pk} · {self.name}'
class OrderUnit(models.Model):
    order=models.ForeignKey(CustomerOrder,on_delete=models.CASCADE,related_name='units'); serial_number=models.CharField(max_length=180,unique=True,db_index=True); aiken_lot=models.CharField(max_length=160,blank=True,db_index=True); aiken_unit_id=models.CharField(max_length=80,blank=True,db_index=True); brand=models.CharField(max_length=160,blank=True); model=models.CharField(max_length=180,blank=True); processor=models.CharField(max_length=180,blank=True); ram=models.CharField(max_length=80,blank=True); disk=models.CharField(max_length=80,blank=True); imported_at=models.DateTimeField(auto_now_add=True)
...
class ComponentReservation(models.Model):
    ...
    repair=models.ForeignKey(Repair,on_delete=models.PROTECT,related_name='component_reservations',null=True,blank=True); unit=models.ForeignKey(OrderUnit,on_delete=models.PROTECT,related_name='component_reservations',null=True,blank=True); component=models.ForeignKey(Component,on_delete=models.PROTECT,related_name='reservations'); technician=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='component_reservations'); installed_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='component_installations',null=True,blank=True); unit_serial_number=models.CharField(max_length=180,db_index=True); status=models.CharField(max_length=16,choices=STATUS,default='active',db_index=True); reserved_at=models.DateTimeField(auto_now_add=True); installed_at=models.DateTimeField(null=True,blank=True); resolved_at=models.DateTimeField(null=True,blank=True); observations=models.TextField(blank=True)
    ...
    def cancel(self):
        ...
        with transaction.atomic():
            ...
            if self.component.inventory_record_id:
                r=self.component.inventory_record; r.status='available'; r.current_sn=''; r.current_technician=''; r.save(update_fields=['status','current_sn','current_technician','updated_at'])
    def install(self,user):
        if self.status!='active': return self.repair
        with transaction.atomic():
            repair=Repair.objects.create(unit=self.unit,repair_type=self.component.component_type,component_type=self.component.component_kind,created_by=user,observations=self.observations); self.repair=repair; self.installed_by=user; self.installed_at=timezone.now(); self.resolved_at=self.installed_at; self.status='installed'; self.save(update_fields=['repair','installed_by','installed_at','resolved_at','status']); self.component.status='installed'; self.component.save(update_fields=['status'])
            if self.component.inventory_record_id:
                r=self.component.inventory_record; r.status='assigned'; r.current_sn=self.unit.serial_number; r.current_technician=user.get_username(); r.save(update_fields=['status','current_sn','current_technician','updated_at'])
            return repair
class RMA(models.Model):
    STATUS=[('open','Abierto'),('review','En evaluación'),('approved','Aprobado'),('rejected','Rechazado'),('closed','Cerrado')]; ORIGINS=[('supplier','Proveedor'),('original','Original de la unidad'),('warehouse','Reserva de almacén')]
    component=models.ForeignKey(Component,on_delete=models.PROTECT,related_name='rmas',null=True,blank=True); component_type=models.ForeignKey(ComponentType,on_delete=models.PROTECT,related_name='rmas',null=True,blank=True); unit=models.ForeignKey(OrderUnit,on_delete=models.PROTECT,related_name='rmas',null=True,blank=True); reservation=models.ForeignKey(ComponentReservation,on_delete=models.PROTECT,related_name='rmas',null=True,blank=True); supplier=models.ForeignKey(Supplier,on_delete=models.PROTECT,related_name='rmas',null=True,blank=True); origin=models.CharField(max_length=16,choices=ORIGINS,default='supplier',db_index=True); reason=models.TextField(blank=True); status=models.CharField(max_length=12,choices=STATUS,default='open',db_index=True); created_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,related_name='rmas_created'); created_at=models.DateTimeField(auto_now_add=True); observations=models.TextField(blank=True)
    def clean(self):
        ...
        if self.origin=='warehouse' and not self.reservation_id and self.component_id:
            ...
            if reservation: self.reservation=reservation; self.unit=reservation.unit
