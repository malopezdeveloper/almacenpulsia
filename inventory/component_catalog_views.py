        ...
        if action == 'create_type':
            ...
            if name:
                with transaction.atomic():
                    ...
                    kind.active = True
                    kind.save(update_fields=['active'])
                    if not hasattr(kind, 'catalog'):
            ...
            field_type = request.POST.get('field_type') or 'text'
            if name:
                key_base = slugify(name).replace('-', '_') or 'campo'
                key = key_base
                n = 2
                while catalog.inventory_table.inventory_fields.filter(key=key).exists():
                    key = f'{key_base}_{n}'
                    n += 1
            ...
            catalog.component_type.active = catalog.active
            catalog.inventory_table.active = catalog.active
            catalog.save(update_fields=['active'])
            catalog.component_type.save(update_fields=['active'])
            catalog.inventory_table.save(update_fields=['active'])
...
def component_catalog_table(request, catalog_pk):
    ...
    records = table.records.all().order_by('internal_id')[:3000]
    rows = [{'record': r, 'values': [r.internal_id if f.is_primary else (r.data or {}).get(f.key, '') for f in fields]} for r in records]
    return render(request, 'inventory/component_catalog_table.html', {
        'catalog': catalog, 'table': table, 'fields': fields, 'rows': rows, 'can_manage_components': _can_manage(request.user)
    })
...
def component_catalog_bulk_create(request, catalog_pk):
    ...
    catalog = get_object_or_404(ComponentCatalog.objects.select_related('component_type', 'inventory_table'), pk=catalog_pk, active=True)
    try:
        quantity = max(1, min(int(request.POST.get('quantity') or 1), 1000))
    except ValueError:
        quantity = 1
    try:
        price = Decimal((request.POST.get('price') or '0').replace(',', '.'))
    ...
    table = catalog.inventory_table
    custom_fields = list(table.inventory_fields.filter(is_primary=False).exclude(key__in=[x[0] for x in COMMON_FIELDS]))
    shared = {field.key: (request.POST.get(f'field_{field.pk}') or '').strip() for field in custom_fields}
    today = timezone.localdate().isoformat()
    created = []
    with transaction.atomic():
        locked = InventoryTable.objects.select_for_update().get(pk=table.pk)
        for _ in range(quantity):
            internal_id = f'{locked.id_prefix}{locked.next_number:0{locked.id_width}d}'
            ...
            data.update({
                ...
                'delivery_date': today,
                'reservation_date': '',
                'technician': request.user.get_username(),
                'destination_sn': '',
            })
            ...
            Component.objects.create(
                component_type=catalog.component_type.name,
                component_kind=catalog.component_type,
            ...
            RecordMovement.objects.create(record=record, movement_type='entry', technician_name=request.user.get_username(), reason='Alta de componente', registered_by=request.user)
            created.append(internal_id)
        locked.save(update_fields=['next_number'])
    messages.success(request, f'{len(created)} componente(s) creados. IDs: {created[0]} → {created[-1]}.')
    return redirect('component_catalog_table', catalog_pk=catalog.pk)
