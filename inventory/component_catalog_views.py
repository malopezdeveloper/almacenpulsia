...
def _new_component_table(name,user):
 ...
 table=InventoryTable.objects.create(name=f'COMPONENTES · {name}',slug=slug,id_header='ID',id_prefix=f'{(base.upper().replace("-","")[:6] or "COMP")}-',id_width=5,next_number=1,created_by=user)
 InventoryField.objects.create(table=table,name='ID',key='id',position=0,field_type='text',is_primary=True);_ensure_common_fields(table);return table
...
def component_catalog_table(request,catalog_pk):
 catalog=get_object_or_404(ComponentCatalog.objects.select_related('component_type','inventory_table'),pk=catalog_pk,active=True);table=catalog.inventory_table;_ensure_common_fields(table);fields=list(table.inventory_fields.all());records=table.records.all().order_by('internal_id')[:3000]
 rows=[{'record':r,'values':[r.internal_id if f.is_primary else (r.data or {}).get(f.key,'') for f in fields],'quantity':int((r.data or {}).get('quantity') or 1)} for r in records];return render(request,'inventory/component_catalog_table.html',{'catalog':catalog,'table':table,'fields':fields,'rows':rows,'can_manage_components':_can_manage(request.user)})
@login_required
@require_POST
def component_catalog_bulk_create(request,catalog_pk):
 ...
 with transaction.atomic():
  ...
  for r in InventoryRecord.objects.select_for_update().filter(table=locked,status='available'):
   rd=r.data or {};same=all(str(rd.get(k,''))==str(v) for k,v in shared.items()) and str(rd.get('price',''))==str(price)
   if same: existing=r;break
  if existing:
   d=dict(existing.data or {});d['quantity']=str(int(d.get('quantity') or 1)+quantity);existing.data=d;existing.save(update_fields=['data']);RecordMovement.objects.create(record=existing,movement_type='entry',technician_name=request.user.get_username(),reason=f'Entrada acumulada +{quantity}',registered_by=request.user);internal_id=existing.internal_id;messages.success(request,f'{quantity} unidad(es) sumadas a {internal_id}. Cantidad total: {d["quantity"]}.')
  else:
...
def component_catalog_adjust_stock(request,catalog_pk,record_pk):
 ...
 with transaction.atomic():
  record=InventoryRecord.objects.select_for_update().get(pk=record.pk);d=dict(record.data or {});current=int(d.get('quantity') or 1);new=current+delta
  ...
  d['quantity']=str(new);record.data=d;record.status='available' if new>0 else 'delivered';record.save(update_fields=['data','status']);RecordMovement.objects.create(record=record,movement_type='entry' if delta>0 else 'delivery',technician_name=request.user.get_username(),reason=f'Ajuste manual de stock {delta:+d}',registered_by=request.user)
