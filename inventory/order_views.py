...
def _allowed(u,p): return u.is_superuser or user_has_permission(u,p) or u.is_staff
...
def order_detail(request,pk):
 if not _allowed(request.user,'orders.view'): return _deny()
 o=get_object_or_404(CustomerOrder.objects.select_related('customer'),pk=pk);units=o.units.all().order_by('serial_number');rs=ComponentReservation.objects.filter(unit__order=o).select_related('component');installed=rs.filter(status__in=['installed','confirmed']);cost=sum((x.component.price or Decimal('0')) for x in installed);source=_aiken_source();aiken_ready=False;aiken_error=''
 if source:
...
def unit_detail(request,pk):
 if not _allowed(request.user,'orders.view'): return _deny()
 u=get_object_or_404(OrderUnit.objects.select_related('order','order__customer','physical_unit'),pk=pk)
 return render(request,'inventory/unit_detail.html',{'unit':u,'reservations':u.component_reservations.select_related('component','technician','installed_by','repair').order_by('-reserved_at'),'repairs':u.repairs.select_related('created_by').order_by('-created_at'),'can_reserve':_allowed(request.user,'components.reserve')})
@login_required
...
def unit_create(request):
 if not _allowed(request.user,'orders.manage'): return _deny()
 if request.method=='POST':
  o=get_object_or_404(CustomerOrder,pk=request.POST['order'],status='open');u=OrderUnit.objects.create(order=o,serial_number=request.POST['serial_number'].strip(),aiken_lot=request.POST.get('aiken_lot','').strip() or o.lot,brand=o.brand,model=o.model,processor=o.processor,ram=o.ram,disk=o.disk);return redirect('unit_detail',pk=u.pk)
...
def reserve_component(request,component_pk):
 messages.info(request,'La reserva directa antigua está desactivada. Selecciona la unidad y usa Bodega / Componentes del Pedido.');return redirect('internal_table',kind='componentes')
@login_required
def reservation_install(request,pk):
 ...
 if r.status=='active':r.install(request.user);messages.success(request,'Componente instalado. La reparación queda pendiente de confirmación desde la intervención de la unidad.')
...
def procurement_resolve(request,pk):
 if request.method!='POST' or not (user_is_purchasing(request.user) or user_has_permission(request.user,'procurement.resolve')):return _deny()
 a=get_object_or_404(ProcurementAlert,pk=pk,status='open');a.status='resolved';a.resolved_at=timezone.now();a.save(update_fields=['status','resolved_at']);return redirect('orders_center')
@login_required
...
def aiken_settings(request):
 ...
 if request.method=='POST':
  candidate=ProductionModelMySQLSource(host=request.POST['host'].strip(),port=int(request.POST.get('port') or 3306),database=request.POST['database'].strip(),username=request.POST['username'].strip(),updated_by=request.user);password=request.POST.get('password','')
  ...
  try:test_source(candidate)
  except Exception as e:messages.error(request,f'CONEXIÓN AIKEN FALLIDA. No se ha guardado ningún cambio. Error: {e}');return render(request,'inventory/aiken_settings.html',{'source':candidate,'connection_status':'error','connection_message':str(e)})
  if request.POST.get('action')=='test':messages.success(request,'CONEXIÓN AIKEN CORRECTA.');return render(request,'inventory/aiken_settings.html',{'source':candidate,'connection_status':'ok','connection_message':'Conexión y esquema Units compatibles.'})
  if s is None:s=ProductionModelMySQLSource()
  s.host=candidate.host;s.port=candidate.port;s.database=candidate.database;s.username=candidate.username;s.encrypted_password=candidate.encrypted_password;s.updated_by=request.user;s.save();messages.success(request,'CONEXIÓN AIKEN CORRECTA Y GUARDADA.');return redirect('aiken_settings')
 status='';msg=''
 if s:
  try:test_source(s);status='ok';msg='Configuración activa comprobada.'
...
@login_required
def aiken_import(request,order_pk):return redirect('order_detail',pk=order_pk)
...
def _report_data(scope,pk):
 ...
 repairs=[r.repair for r in rs if r.repair_id];cost=sum((r.component.price or Decimal('0')) for r in rs if r.status in ('installed','confirmed'));return title,subject,units,rs,repairs,cost
@login_required
...
def _safe_sql(sql):
 s=re.sub(r'/\*.*?\*/',' ',sql,flags=re.S);s=re.sub(r'--.*?$',' ',s,flags=re.M).strip().rstrip(';').strip();return bool(re.match(r'^(select|with)\b',s,re.I)) and ';' not in s and not re.search(r'\b(insert|update|delete|drop|alter|create|replace|truncate|attach|detach|pragma|vacuum|grant|revoke|call|execute)\b',s,re.I)
def _execute_query(q):
 ...
 with connection.cursor() as c:c.execute(q.sql);cols=[x[0] for x in c.description];rows=c.fetchmany(5000)
 return cols,rows
@login_required
